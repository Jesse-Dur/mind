...
async function classifyAndStore(input: string) {
  const ollamaUrl = process.env.OLLAMA_URL ?? "http://localhost:11434"
  const tiles = tilesDb.list()
  const tags = tagsDb.list()
  ...
  const inputTagHint = inputTags.length
    ? `\nDetected tags in input: ${inputTags.map(t => {
        const taggedThoughts = thoughtsDb.list().filter(th => th.tags.includes(t.name))
        if (!taggedThoughts.length) { const rTile = tiles.find(ti => ti.id === randomTileId); return `"${t.name}" (no existing thoughts — use tile "${rTile?.title ?? "Random"}" id:${randomTileId ?? "unknown"})` }
...
- Call done() in the SAME message as your last action, not separately.`,
  ...
  for (let i = 0; i < 8; i++) {
    ...
    if (!data.message) {
      ...
      messages.push({ role: "user", content: `Error: ${data.error}. Try again.` })
      continue
    }

    const msg = data.message as { role: string; content: string; thinking?: string; tool_calls?: { function: { name: string; arguments: Record<string, unknown> } }[] }
    log(`💬 ${msg.thinking ? `[thinking] ${String(msg.thinking).slice(0, 100)}` : ""}${msg.tool_calls?.length ? `→ ${msg.tool_calls.map(c => c.function.name).join(", ")}` : msg.content?.slice(0, 150)}`)

    if (!msg.tool_calls?.length) {
      // model responded with text instead of done() — treat as finished
      log(`✅ Done: ${historyActions.join(" | ") || "no actions"}`)
      if (historyActions.length > 0) {
        historyDb.log("ai.process",
    ...
    for (const call of msg.tool_calls) {
      ...
      if (call.function.name === "search_thoughts") {
        ...
        searchCount++
        result = results.length ? `Found: ${JSON.stringify(results)}. NOW call update_thought/delete_thought/move_thought immediately. Do not search again.` : `No results for "${query}". Try search_by_tag or search_by_tile instead.`
        ...
        const results = thoughtsDb.list(tileId)
          .slice(0, 20)
          .map((t) => ({ id: t.id, content: t.content, tags: t.tags }))
        const tileName = tiles.find(t => t.id === tileId)?.title ?? tileId
        log(`📂 search_by_tile(${tileId} "${tileName}") → ${results.length} result(s)`)
        searchCount++
        result = results.length ? `Found: ${JSON.stringify(results)}. NOW call update_thought/delete_thought/move_thought immediately. Do not search again.` : `No thoughts in tile ${tileId}`
        ...
        const thought = thoughtsDb.create({ tile_id: tileId, content, tags: validTags, sort_order: 0 }, true)
        ...
        thoughtsDb.update(id, content, validTags)
      ...
      messages.push({ role: "tool", content: result, name: call.function.name })
    }

    // remind model of what it has already done this session
    if (historyActions.length > 0) {
...
ollamaRoute.post("/process", async (c) => {
  ...
  if (running < max) {
    running++
    classifyAndStore(input).catch(console.error).finally(() => running--)
  } else {
    setTimeout(() => {
      running++
      classifyAndStore(input).catch(console.error).finally(() => running--)
